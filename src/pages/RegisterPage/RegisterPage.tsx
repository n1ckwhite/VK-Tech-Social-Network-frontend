...
export const RegisterPage: FC = () => {
    ...
    const registerUser = async (e: SyntheticEvent) => {
       ...
       await register({name,email,password}).unwrap().then((data) => {
        ...
        <div className={styleLoginAndRegPage.flex}>
            <div className={styleLoginAndRegPage.div}>
                ...
                <form onSubmit={registerUser} className={styleLoginAndRegPage.form}>
                    ...
                    <Input classname={error ? styleLoginAndRegPage.err : ''} type="text" id="name" placeholder="не менее 5 символов" value={name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setName(e.target.value)} required/>
                    ...
                    <Button classname={errMsg ? styleLoginAndRegPage.mt0 : styleLoginAndRegPage.btn} text="Регистрация"/>
                </form>
                <Link className={styleLoginAndRegPage.link} to="/login">Войти</Link>
            </div>
        </div>
    )
}