...
export const SearchPage: FC = () => {
    ...
    const resultUsers = async (e: SyntheticEvent) => {
        ...
        if(nameU.length !== 0 || nameU !== '') {
            await searchUsers({name: nameU}).unwrap().then((response) => {
        ...
        <Header/>
            ...
            <div className={styleSearchPage.block}>
                ...
                <form onSubmit={resultUsers} className={styleSearchPage.form}>
                    <Label classname={styleSearchPage.label} htmlFor="name" text="Введите имя пользователя"/>
                    <Input value={nameU} onChange={(e) => setNameU(e.target.value)} classname={styleSearchPage.input} placeholder="Например: nick white" id="name" type="text" required/>
                    {  errMsg.length !== 0 ?  <p className={styleSearchPage.err}>{errMsg}</p> : ''}
                    <Button classname={styleSearchPage.btn} text="Поиск"/>
                </form>
                {resultUser.length !== 0 ? <p className={styleSearchPage.result}>Результат: </p> : ''}
